...
(define (old-/-two x1 x2) (/ x1 x2))
(define old-/ /)
(define (old-sqrt x) (sqrt x))
(define (old-exp x) (exp x))
(define (old-log x) (log x))
(define (old-expt x1 x2) (expt x1 x2))
(define (old-sin x) (sin x))
(define (old-cos x) (cos x))
(define (old-atan x1 x2) (atan x1 x2))
(define (old-=-two x1 x2) (= x1 x2))
(define old-= =)
(define (old-<-two x1 x2) (< x1 x2))
(define old-< <)
...
(define (old->=-two x1 x2) (>= x1 x2))
(define old->= >=)
(define (old-zero? x) (zero? x))
(define (old-positive? x) (positive? x))
(define (old-negative? x) (negative? x))
(define (old-real? x) (real? x))
...
(define (+-two x1 x2)
 (if (and (number? x1) (number? x2))
     (old-+-two x1 x2)
     (lift-real*real->real
      old-+-two (lambda (x1 x2) 1.0) (lambda (x1 x2) 1.0) x1 x2)))

(define (+ . xs)
 (if (null? xs)
     (old-+)
     (let loop ((xs (cdr xs)) (c (car xs)))
      (if (null? xs) c (loop (cdr xs) (+ c (car xs)))))))
...
(define (- . xs)
 (cond ((null? xs) (old--))
       ((null? (cdr xs)) (- (car xs)))
       (else (let loop ((xs (cdr xs)) (c (car xs)))
	      (if (null? xs) c (loop (cdr xs) (- c (car xs))))))))
...
(define (*-two x1 x2)
 (if (and (number? x1) (number? x2))
     (old-*-two x1 x2)
     (lift-real*real->real
      old-*-two (lambda (x1 x2) x2) (lambda (x1 x2) x1) x1 x2)))
...
(define (positive? x)
 (if (number? x)
     (old-positive? x)
     (old-positive? (primal* x))))

(define (negative? x)
 (if (number? x)
     (old-negative? x)
     (old-negative? (primal* x))))
...
(define (dot u v) (map-reduce-vector +-two 0 *-two u v))
...
(define (mean-angle angles)
 (atan (map-reduce +-two 0 sin angles) (map-reduce +-two 0 cos angles)))
  ...
  (map-n-vector
   (lambda (j)
    (let* ((zj (matrix-column-ref z j))
	   (zj-sum (reduce-vector +-two zj 0.0))
	   ;; Optimize values.
	   (mu (k*v (/ zj-sum)
	   ...
	   (sigma (clip-eigenvalues
		   (k*m (/ zj-sum)
			(map-reduce-vector
			 ...
			 (lambda (zij xi)
			  (k*m zij (self-outer-product *-two (v- xi mu))))
			 zj
			 x))
		   clip)))
 ...
 (map-n-vector
  (lambda (i)
   (let ((zi (map-n-vector (lambda (j) (+ (/ jj) (noise (/ jj)))) jj)))
    (k*v (/ (reduce-vector +-two zi 0.0)) zi)))
  ii))
...
(define (tree->tree-node tree)
 ...
 (let ((daughters (map tree->tree-node (rest (rest (rest tree)))))
  ...
  (make-tree-node
   (max (map-reduce +-two 0 tree-node-width daughters)
	(+ (xtextwidth
	    (if bold? *bold-font* *roman-font*) text (string-length text))
	   10))
   (if (null? daughters)
   ...
   (if (or (null? daughters)
	   (< (map-reduce +-two 0 tree-node-width daughters)
	...
	2)
       (+ (quotient
	   (- (+ (map-reduce +-two 0 tree-node-width (rest (reverse daughters)))
...
(define (multivariate-argmax-F f x)
 (multivariate-argmin-F (lambda (x) (- (f x))) x))
...
(define (multivariate-argmax-R f x)
 (multivariate-argmin-R (lambda (x) (- (f x))) x))
