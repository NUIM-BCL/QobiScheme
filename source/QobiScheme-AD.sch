...
(define-external +-two top-level)
(define-external + top-level)
(define-external --two top-level)
(define-external - top-level)
(define-external *-two top-level)
(define-external * top-level)
(define-external /-two top-level)
(define-external / top-level)
(define-external sqrt top-level)
(define-external exp top-level)
(define-external log top-level)
(define-external expt top-level)
(define-external sin top-level)
(define-external cos top-level)
(define-external atan top-level)
(define-external =-two top-level)
(define-external = top-level)
(define-external <-two top-level)
(define-external < top-level)
...
(define-external >=-two top-level)
(define-external >= top-level)
(define-external zero? top-level)
(define-external positive? top-level)
(define-external negative? top-level)
(define-external real? top-level)

(define-external (+-two x1 x2) QobiScheme)
(define-external (+ . xs) QobiScheme)
(define-external (--two x1 x2) QobiScheme)
(define-external (- . xs) QobiScheme)
(define-external (*-two x1 x2) QobiScheme)
(define-external (* . xs) QobiScheme)
(define-external (/-two x1 x2) QobiScheme)
(define-external (/ . xs) QobiScheme)
(define-external (sqrt x) QobiScheme)
(define-external (exp x) QobiScheme)
(define-external (log x) QobiScheme)
(define-external (expt x1 x2) QobiScheme)
(define-external (sin x) QobiScheme)
(define-external (cos x) QobiScheme)
(define-external (atan . xs) QobiScheme)
(define-external (=-two x1 x2) QobiScheme)
(define-external (= . xs) QobiScheme)
(define-external (<-two x1 x2) QobiScheme)
(define-external (< . xs) QobiScheme)
...
(define-external (>=-two x1 x2) QobiScheme)
(define-external (>= . xs) QobiScheme)
(define-external (zero? x) QobiScheme)
(define-external (positive? x) QobiScheme)
(define-external (negative? x) QobiScheme)
(define-external (real? x) QobiScheme)
...
(define-macro +
 (lambda (form expander)
  (case (length form)
   ...
   ((3) (expander `((lambda (x1 x2) (+-two x1 x2)) ,(cadr form) ,(caddr form))
		  expander))
   (else (expander `(+ ,(cadr form) (+ ,@(cddr form))) expander)))))

(define-macro -
 (lambda (form expander)
  (case (length form)
   ((1) (expand-error '- form))
   ((2) (expander `(- 0 ,(cadr form)) expander))
   ((3) (expander `((lambda (x1 x2) (--two x1 x2)) ,@(cdr form)) expander))
   (else (expander `(- (- ,(cadr form) ,(caddr form)) ,@(cdddr form))
		   expander)))))
...
(define-external (lift-real*real->real f df/dx1 df/dx2 x1 x2) QobiScheme)
(define-external (lift-real->real f df/dx x) QobiScheme)

(define-macro =
 (lambda (form expander)
  `(,(if (= (length form) 3)
	 (expander '(lambda (x1 x2) (=-two x1 x2)) expander)
	 '=)
...
(define-external (primal* p) QobiScheme)
(define-structure-external QobiScheme
 dual-number epsilon primal perturbation)
(define-structure-external QobiScheme
 tape epsilon primal factors tapes fanout sensitivity)
(define-external *e* QobiScheme)
(define-external (<_e e1 e2) QobiScheme)
(define-external (dual-number e x x-prime) QobiScheme)
(define-external (tape e primal factors tapes) QobiScheme)
(define-external (derivative-F f) QobiScheme)
(define-external (replace-ith x i xi) QobiScheme)
(define-external (gradient-F f) QobiScheme)
(define-external (determine-fanout! tape) QobiScheme)
(define-external (reverse-phase! sensitivity tape) QobiScheme)
(define-external (gradient-R f) QobiScheme)
(define-external (derivative-R f) QobiScheme)
(define-external (write-real x) QobiScheme)
(define-external (multivariate-argmin-F f x) QobiScheme)
(define-external (multivariate-argmax-F f x) QobiScheme)
(define-external (multivariate-max-F f x) QobiScheme)
(define-external (multivariate-argmin-R f x) QobiScheme)
(define-external (multivariate-argmax-R f x) QobiScheme)
(define-external (multivariate-max-R f x) QobiScheme)
